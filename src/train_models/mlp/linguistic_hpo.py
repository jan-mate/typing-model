...
from src.utils.data_loader import MLP_LINGUISTIC_FEATURES, prepare_sequential_data, get_train_val_masks
from src.utils.training_utils import SEED, seed_everything, seed_worker
warnings.filterwarnings('ignore')

CFG = {
    "data_path": ENRICHED_DATA_PATH,
    "model_save_dir": model_dir("mlp_linguistic", subdir=""),
    "target": "iki_z",
    "subset": None,
    "fold_id": 1,
...
ACTIVE_FEATURES = MLP_LINGUISTIC_FEATURES
...
def main():
    ...
    db_path = os.path.join(CFG["model_save_dir"], "optuna_linguistic.db")
    ...
    if not CFG["RUN_PHASE_2"]:
        for w_b in CFG["w_back_grid"]:
            for w_a in CFG["w_ahead_grid"]:
                study = optuna.create_study(study_name=f"linguistic_w{w_b}_a{w_a}", storage=f"sqlite:///{db_path}", load_if_exists=True, direction="minimize", pruner=pruner, sampler=sampler)
        ...
        final_study = optuna.create_study(study_name=f"linguistic_w{best_w_b}_a{best_w_a}", storage=f"sqlite:///{db_path}", load_if_exists=True, direction="minimize", pruner=pruner, sampler=sampler)
        ...
        with open(os.path.join(CFG["model_save_dir"], "best_linguistic_params.json"), "w") as f: json.dump(final_params, f, indent=4)
