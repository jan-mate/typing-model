...
from src.models.wrappers import MLP_LINGUISTIC_FEATURES
...
CFG = {
    ...
    "model_save_dir": model_dir("mlp_linguistic"),
    "subset": None,
    "epochs": 40,
    "patience": 5,
...
def main():
    ...
    json_path = os.path.join(os.path.dirname(CFG["model_save_dir"]), "best_linguistic_params.json")
    ...
    for fold_id in tqdm(CFG["folds_to_train"], desc="Overall Folds Progress"):
        model_path = os.path.join(ensemble_dir, f"final_mlp_linguistic_fold_{fold_id}.pth")
        scaler_path = os.path.join(ensemble_dir, f"final_mlp_linguistic_scaler_fold_{fold_id}.pkl")
        done_path = os.path.join(ensemble_dir, f"final_mlp_linguistic_fold_{fold_id}.done")

        if os.path.exists(done_path):
            continue

        print(f"\n--- Training MLP Linguistic on Fold {fold_id} ---")
    ...
    if fold_scores:
        print(f"\nAverage MLP Linguistic MAE of newly trained folds: {np.mean(fold_scores):.6f}")
